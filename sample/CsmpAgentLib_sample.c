...
/**
 * @brief interface description function that reads actual hardware interfaces
 *
...
void* interface_desc_get(uint32_t *num) {
  //Variable for interface discovery
  struct ifaddrs *ifaddr, *ifa;
  int family, n, max_interfaces = MAX_INTERFACES;
  char host[NI_MAXHOST];
  int interface_count = 0;

  // Get list of of all interfaces
  if(getifaddrs(&ifaddr) == -1) {
    perror("getifaddrs");
    // Reverting back to hardcoded values as getifaddrs has failed
    return fallback_interface_desc_get(num);
  }
  // First pass will count the number of interfaces
  for (ifa = ifaddr, n = 0; ifa != NULL && interface_count < max_interfaces; ifa = ifa->ifa_next, n++) {
    ...
    // only count the interfaces as we would have multiple families, but we are only interested in IPv6
    if (family == AF_INET6) {
      bool already_counted = false;
      for (int i = 0; i < interface_count; i++) {
        if (strcmp(g_interfaceDesc[i].ifname, ifa->ifa_name) == 0) {
  ...
  *num = interface_count;
  ...
  //Second pass will populate interface data
  interface_count = 0;
  for (ifa = ifaddr; ifa != NULL && interface_count < max_interfaces; ifa = ifa->ifa_next) {
    ...
    // Set common fields
    g_interfaceDesc[interface_count].has_ifindex = true;
    g_interfaceDesc[interface_count].has_ifname = true;
    g_interfaceDesc[interface_count].has_ifdescr = true;
    g_interfaceDesc[interface_count].has_iftype = true;
    // Get interface index and set it
    g_interfaceDesc[interface_count].ifindex = if_nametoindex(ifa->ifa_name);
    // Set Interface name
    sprintf(g_interfaceDesc[interface_count].ifname, "%s", ifa->ifa_name);
    // Set interface description (could be enhanced with more detailed info)
    if (strncmp(ifa->ifa_name, "lo", 2) == 0) {
      sprintf(g_interfaceDesc[interface_count].ifdescr, "Loopback");
      g_interfaceDesc[interface_count].iftype = 24; // Loopback
    } else if (strncmp(ifa->ifa_name, "eth", 3) == 0 || 
               strncmp(ifa->ifa_name, "en", 2) == 0) {
      sprintf(g_interfaceDesc[interface_count].ifdescr, "Ethernet");
      g_interfaceDesc[interface_count].iftype = 6; // Ethernet
    } else if (strncmp(ifa->ifa_name, "wlan", 4) == 0 || 
               strncmp(ifa->ifa_name, "wl", 2) == 0) {
      sprintf(g_interfaceDesc[interface_count].ifdescr, "Wireless");
      g_interfaceDesc[interface_count].iftype = 71; // IEEE 802.11
    } else if (strncmp(ifa->ifa_name, "lowpan", 6) == 0) {
      sprintf(g_interfaceDesc[interface_count].ifdescr, "Ieee154");
      g_interfaceDesc[interface_count].iftype = 259; // Ieee154
      
      // For lowpan interfaces, set physical address
      g_interfaceDesc[interface_count].has_ifphysaddress = true;
      g_interfaceDesc[interface_count].ifphysaddress.len = 8;
      memcpy(g_interfaceDesc[interface_count].ifphysaddress.data, g_eui64, sizeof(g_eui64));
    } else {
      sprintf(g_interfaceDesc[interface_count].ifdescr, "Other");
      g_interfaceDesc[interface_count].iftype = 1; // Other
    }

    // Get mac address for non-loopback addresses
    if (strcmp(ifa->ifa_name, "lo") != 0) {
      ...
      if (fd >= 0) {
        ...
        if (ioctl(fd, SIOCGIFHWADDR, &ifr) >= 0) {
          g_interfaceDesc[interface_count].has_ifphysaddress = true;
          g_interfaceDesc[interface_count].ifphysaddress.len = 6;
          memcpy(g_interfaceDesc[interface_count].ifphysaddress.data, ifr.ifr_hwaddr.sa_data, 6);
        }
        close(fd);
      }
    }
...
/**
 * @brief fallback function for interface description function
 *
 * @param num  amount of instances (array index) of interface Descriptions
 * @return void* pointer to global variable g_interfaceDesc
 */
void* fallback_interface_desc_get(uint32_t *num) {
  DPRINTF("## fallback_interface_desc_get");
  *num = 2;
...
void* ipaddress_get(uint32_t *num) {
  ...
  char host[NI_MAXHOST];
  int ip_count = 0;
  int max_addresses = MAX_IP_ADDRESSES;

  if (getifaddrs(&ifaddr) == -1) {
    perror("getifaddrs");
    return failover_ipaddress_get(num);
  }
  // First loop will count the number of IP addresses
  for(ifa = ifaddr; ifa != NULL && ip_count < max_addresses; ifa = ifa->ifa_next) {
    if (ifa->ifa_addr == NULL) {
      continue;
    }
    family = ifa->ifa_addr->sa_family;

    // We are only interested in IPv6 family
    if(family == AF_INET6) {
      ip_count++;
    }
  }
  ...
  *num = ip_count;
  ...
  ip_count = 0;
  //second pass will actually populate the global variable for the ip addresses
  for (ifa = ifaddr; ifa != NULL && ip_count < max_addresses; ifa = ifa->ifa_next) {
    if (ifa->ifa_addr == NULL) {
      continue;
    }
    ...
    family = ifa->ifa_addr->sa_family;
    //again we are only interested in the IPv6 addresses
    if (family == AF_INET6) {
      g_ipAddress[ip_count].has_ipaddressindex = true;
      g_ipAddress[ip_count].has_ipaddressaddrtype = true;
      g_ipAddress[ip_count].has_ipaddressaddr = true;
      g_ipAddress[ip_count].has_ipaddressifindex = true;
      g_ipAddress[ip_count].has_ipaddresstype = true;
      g_ipAddress[ip_count].has_ipaddressorigin = true;
      g_ipAddress[ip_count].has_ipaddressstatus = true;
      g_ipAddress[ip_count].has_ipaddresspfxlen = true;

    // setting the index
    g_ipAddress[ip_count].ipaddressindex = ip_count + 1;
    
    // setting the address type
    g_ipAddress[ip_count].ipaddressaddrtype = IPV6;

    //getting and setting the interface index
    g_ipAddress[ip_count].ipaddressifindex = if_nametoindex(ifa->ifa_name);

    //get ip address
    g_ipAddress[ip_count].ipaddressaddr.len = 16;
    memcpy(g_ipAddress[ip_count].ipaddressaddr.data, &((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr, 16);

    // determine address type (unicast, multicast, etc) for our example we will just assume UNICAST :)
    g_ipAddress[ip_count].ipaddresstype = UNICAST;

    // determine address origin
    if (IN6_IS_ADDR_LINKLOCAL(&((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr)) {
      g_ipAddress[ip_count].ipaddressorigin = LINKLAYER;
      g_ipAddress[ip_count].ipaddresspfxlen = 64;
    } else if(IN6_IS_ADDR_LOOPBACK(&((struct sockaddr_in6 *)ifa->ifa_addr)->sin6_addr)) {
      g_ipAddress[ip_count].ipaddressorigin = OTHER;
      g_ipAddress[ip_count].ipaddresspfxlen = 128;
    } else {
      g_ipAddress[ip_count].ipaddressorigin = DHCP;
      g_ipAddress[ip_count].ipaddresspfxlen = 64;
    }

    g_ipAddress[ip_count].ipaddressstatus = true;
...
void* failover_ipaddress_get(uint32_t *num) {
  int i;
...
void* interface_metrics_get(uint32_t *num) {
  ...
  int family;
  int if_count = 0;
  int max_interfaces = MAX_INTERFACES;

  //get list of interfaces
  if(getifaddrs(&ifaddr) == -1) {
    perror("getifaddrs");
    return fallback_interface_metrics_get(num);
  }

  // First pass will count the number of interfaces
  for (ifa = ifaddr; ifa != NULL && if_count < max_interfaces; ifa = ifa->ifa_next) {
    if (ifa->ifa_addr == NULL) {
      continue;
    }

    family = ifa->ifa_addr->sa_family;
    // again we only care about the IPv6 addresses 
    if (family == AF_INET6) {
      bool already_counted = false;
      for (int i = 0; i < if_count; i++) {
        if (g_interfaceMetrics[i].ifindex == if_nametoindex(ifa->ifa_name)) {
          already_counted = true;
          break;
        }
      }
  ...
  *num = if_count;
  ...
  if_count = 0;
  //second pass will actual populate metrics based on the interfaces
  if_count = 0;
  for (ifa = ifaddr; ifa != NULL && if_count < max_interfaces; ifa = ifa->ifa_next) {
    if(ifa->ifa_addr == NULL) {
      continue;
    ...
    if (family == AF_INET6) {
      ...
      g_interfaceMetrics[if_count].has_ifindex = true;
      g_interfaceMetrics[if_count].has_ifadminstatus = true;
      g_interfaceMetrics[if_count].has_ifoperstatus = true;
      g_interfaceMetrics[if_count].has_iflastchange = true;
      g_interfaceMetrics[if_count].has_ifinoctets = true;
      g_interfaceMetrics[if_count].has_ifoutoctets = true;
      g_interfaceMetrics[if_count].has_ifindiscards = true;
      g_interfaceMetrics[if_count].has_ifinerrors = true;
      g_interfaceMetrics[if_count].has_ifoutdiscards = true;
      g_interfaceMetrics[if_count].has_ifouterrors = true;

      //setting the interface index for the interfaceMetrics 
      g_interfaceMetrics[if_count].ifindex = if_nametoindex(ifa->ifa_name);

      //getting the interface statistics from /proc/net/dev
      ...
      if (fp) {
        //skip the first 2 headers lines as they are simply informational
        ...
        while (fgets(line, sizeof(line), fp)) {
          ...
          sscanf(ptr, "%[^:]: %llu %llu %llu %llu %*u %*u %*u %*u %llu %llu %llu %llu", if_name, &rx_bytes, &rx_packets, &rx_errs, &rx_drop, &tx_bytes, &tx_packets, &tx_errs, &tx_drop);

          // removing trailing white space from the ifname
          char *end = if_name + strlen(if_name) - 1;
          while (end > if_name && isspace(*end)) {
            *end = '\0';
            end--;
          }

          if (strcmp(if_name, ifa->ifa_name) == 0) {
            g_interfaceMetrics[if_count].ifinoctets = rx_bytes;
            g_interfaceMetrics[if_count].ifoutoctets = tx_bytes;
            g_interfaceMetrics[if_count].ifinerrors = rx_errs;
            g_interfaceMetrics[if_count].ifouterrors = tx_errs;
            g_interfaceMetrics[if_count].ifindiscards = rx_drop;
            g_interfaceMetrics[if_count].ifoutdiscards = tx_drop;
            break;
          }
        }
        fclose(fp);
      ...
      if (ifa->ifa_flags & IFF_UP) {
        g_interfaceMetrics[if_count].ifadminstatus = IF_ADMIN_STATUS_UP;
        if (ifa->ifa_flags & IFF_RUNNING) {
          g_interfaceMetrics[if_count].ifoperstatus = IF_OPER_STATUS_UP;
        } else {
          g_interfaceMetrics[if_count].ifoperstatus = IF_OPER_STATUS_DOWN;
        }
      } else {
        g_interfaceMetrics[if_count].ifadminstatus = IF_ADMIN_STATUS_DOWN;
        g_interfaceMetrics[if_count].ifoperstatus = IF_OPER_STATUS_DOWN;
      }

      // setting last change time so that we can keep track since last change duhhh
      g_interfaceMetrics[if_count].iflastchange = 0;
 ...
 */
void* fallback_interface_metrics_get(uint32_t *num) {
  static int outnum = 0;
  static int innum = 0;
  int i;
